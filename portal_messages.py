...
/* WHOSE VOICE THIS IS.
   Every message that was not the student's own looked identical — the same
   avatar, the same two letters — whether their counsellor had typed it or the
   system had sent it. `from` carries the name now; anything without one is
   the office speaking, which is true of every automatic notice. */
...
function bubble(m) {
  ...
  return '<div style="display:flex;gap:10px;' + (mine ? 'flex-direction:row-reverse' : '') + '">' +
    '<span style="width:32px;height:32px;border-radius:50%;flex:none;display:grid;place-items:center;' +
      ...
      (mine ? 'You' : initialsOf(m.from)) + '</span>' +
    '<div style="max-width:74%">' +
      ...
      '<div style="font:400 10.4px/1.6 var(--sans);color:var(--muted);margin-top:3px;' +
        ...
        (mine ? '' : '<b style="font-weight:700;color:var(--navy-700)">' + esc(m.from || 'Glovels') +
          '</b> \u00b7 ') + stamp(m.at) + '</div>' +
    '</div></div>';
}
