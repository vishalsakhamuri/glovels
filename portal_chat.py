...
connectLive({
  chat: async () => {
    await loadList();
    /* A chat is written into the enquiries book too, so the count on the tab
       goes stale the moment one arrives unless it is read again here. */
    loadEnquiries();
    if (openId) {
  ...
  $('#enqRows').innerHTML = r.enquiries.map(e =>
    '<tr>' +
      '<td><b>' + esc(e.name || 'no name') + '</b>' +
        (e.note ? '<br><span style="font-size:11.8px;color:var(--navy-800);' +
          'font-weight:600">' + esc(e.note) + '</span>' : '') +
