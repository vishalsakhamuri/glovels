...
BODY = """
    ...
    <section class="pane" id="t-dest">
      ...
      <div class="p-card" style="padding:0;overflow-x:auto">
        <table class="tbl" style="margin:0">
          <thead><tr><th>Destination</th><th>Code</th><th style="text-align:right;padding-right:22px">Programmes</th><th>Status</th>
...
/* "Yes, save it" for a deadline that has passed — one press, one save. */
let PAST_OK = false;
...
function paintDests() {
  ...
  /* In order: the destinations with the most programmes first, then by name,
     so the Programmes column reads down and the ones with nothing in them
     sit together at the foot. This was the order they were added in, which
     put 3320, 11142, 2, 4, 2, 1, 2191 down one column. The finder's own
     order (the office's sort) is untouched. */
  const inOrder = DESTS.slice().sort((a, b) => (b.programmes - a.programmes) || a.name.localeCompare(b.name));
  $('#destRows').innerHTML = inOrder.map(d =>
    ...
    '<td style="text-align:right;font-variant-numeric:tabular-nums;padding-right:22px">' + d.programmes.toLocaleString('en-IN') + '</td>' +
    ...
    '<td>' + (() => {
      ...
      return '<button type="button" class="btn btn-ghost btn-sm" style="min-width:132px" data-dreq="' + esc(d.code) + '">' +
...
/* What a programme asks for, beyond a grade. The list mirrors server/reqs.js
   FIELDS — key, label, note, type — and the server is the one that refuses;
   this only draws the boxes and sends what was typed. */
const REQ_FIELDS = [
  ['specialisation', 'Specialisation', 'text', 'Narrower than the field. "Embedded Systems", not "Electrical Engineering".', 'placeholder="Embedded Systems" maxlength="80"'],
  ...
  ['bachelorSubjects', 'Bachelor’s in', 'text', 'What the bachelor’s has to be in. "Electrical Engineering, Electronics or closely related".', 'placeholder="Computer Science, IT or closely related" maxlength="200"'],
  ['bachelorYears', 'Bachelor’s length (years)', 'years', '3, 4 or 5. The one that quietly turns down a three-year BSc. 210 ECTS means 4 here.', ''],
  ['workExpRequired', 'Work experience required', 'yesno', 'Blank if not stated.', ''],
  ['workExpMonths', 'Work experience (months)', 'int', 'A floor, in months. Blank when only "required" is known.', 'type="number" min="0" max="120" step="1" placeholder="24"'],
  ['papersRequired', 'Paper publication required', 'yesno', 'Blank if not stated.', ''],
];
function reqInputs(reqs) {
  const r = reqs || {};
  const yn = [['', 'Not stated'], ['yes', 'Yes'], ['no', 'No']];
  return REQ_FIELDS.map(([key, label, type, note, attrs]) => {
    const v = r[key];
    const id = 'fReq_' + key;
    if (type === 'yesno') return select(label, id, v === true ? 'yes' : v === false ? 'no' : '', yn, note);
    if (type === 'level') return select(label, id, v == null ? '' : v,
      [['', 'Not stated'], ['none', 'None — taught in English'], ['A1', 'A1'], ['A2', 'A2'], ['B1', 'B1'], ['B2', 'B2'], ['C1', 'C1'], ['C2', 'C2']], note);
    if (type === 'years') return select(label, id, v == null ? '' : String(v),
      [['', 'Not stated'], ['3', '3 years'], ['4', '4 years'], ['5', '5 years']], note);
    return field(label, id, v == null ? '' : v, attrs, note);
  }).join('');
}
/* Blank goes up as null — "not stated" — never as 0 or "". */
function readReqs() {
  const out = {};
  REQ_FIELDS.forEach(([key, , type]) => {
    const el = $('#fReq_' + key); if (!el) return;
    const s = String(el.value).trim();
    if (s === '') { out[key] = null; return; }
    if (type === 'yesno') out[key] = s === 'yes';
    else if (type === 'text' || type === 'level') out[key] = s;
    else out[key] = s;    /* numbers travel as typed; the server bounds them */
  });
  return out;
}
...
function openEditor(p) {
  ...
  $('#pmBody').innerHTML =
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:0 14px">' +
      field('Programme name', 'fProgram', v.program, 'placeholder="MSc Data Science" maxlength="140"') +
      field('University', 'fUni', v.university, 'placeholder="University of Melbourne" maxlength="140"') +
      select('Destination', 'fCountry', v.country,
        [['', 'Choose…']].concat(DESTS.filter(d => d.active).map(d => [d.code, (d.flag || '') + ' ' + d.name]))) +
      field('City', 'fCity', v.city, 'placeholder="Melbourne" maxlength="80"') +
    ...
    '<h3 style="font-size:14px;margin:18px 0 4px">What it asks for</h3>' +
    '<p style="margin:0 0 10px;font-size:11.8px;color:var(--muted);line-height:1.5">' +
      'The finder reads these to say whether a student clears this programme. ' +
      '<b>Leave a box empty when the university has not stated it</b> &mdash; an empty box ' +
      'never rules a student out, a wrong number does. These are the same columns as on ' +
      'the catalogue sheet, and a DAAD sheet fills them in bulk.</p>' +
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:0 14px">' +
      reqInputs(v.reqs) +
    '</div>' +
...
document.addEventListener('click', async e => {
  ...
  if (e.target.closest('#pmPastOk')) { PAST_OK = true; $('#pmSave').click(); return; }
  if (e.target.closest('#pmPastNo')) {
    $('#pmErr').style.display = 'none';
    const d = $('#fDeadline0'); if (d) d.focus();
    return;
  }
  if (e.target.closest('#pmSave')) {
    ...
    if (editing && editing.reqFor) {
      ...
      $('#pmErr').style.display = 'none';
      /* The same bounds the server holds, said first and beside the box. A
         minus sign or a 99 is not tidied into a number nobody typed. */
      const rbad = [];
      const rbound = (id, label, lo, hi) => {
        const el = $('#' + id); if (!el) return;
        el.removeAttribute('aria-invalid'); el.style.borderColor = '';
        const raw = el.value.trim(); if (!raw) return;
        const n = Number(raw.replace(/[,\s₹]/g, ''));
        if (!Number.isFinite(n) || n < lo || n > hi) {
          rbad.push(label + ' has to be between ' + lo.toLocaleString('en-IN') + ' and ' + hi.toLocaleString('en-IN') + ' — you entered ' + raw + '.');
          el.setAttribute('aria-invalid', 'true'); el.style.borderColor = '#c0392b';
          if (rbad.length === 1) el.focus();
        }
      };
      rbound('rCgP', 'Minimum CGPA — public', 0, 10);
      rbound('rCgV', 'Minimum CGPA — private', 0, 10);
      rbound('rFunds', 'Funds to show', 0, 99999999);
      rbound('rLiving', 'Living costs', 0, 99999999);
      if (rbad.length) {
        $('#pmErr').textContent = 'Not saved. ' + rbad.join(' ');
    ...
    const text = (id, label, max) => {
      const el = $('#' + id); if (!el) return;
      el.removeAttribute('aria-invalid'); el.style.borderColor = '';
      const v = el.value.trim(); if (!v) return;
      const run = /\S{41,}/.exec(v);
      const why = v.length > max ? label + ' is ' + v.length + ' characters; the most the site shows is ' + max + '.'
        : run ? label + ' has a run of ' + run[0].length + ' characters without a space — that is not a name.' : '';
      if (!why) return;
      bad.push(why); el.setAttribute('aria-invalid', 'true'); el.style.borderColor = '#c0392b';
      if (bad.length === 1) el.focus();
    };
    text('fProgram', 'The programme name', 140);
    text('fUni', 'The university name', 140);
    text('fCity', 'The city', 80);
    if (bad.length) {
    ...
    /* A deadline that has already gone by is asked about, once. The year is
       not used — the site shows the next 31 August — but "31 Aug 2026" typed
       on the 12th of September 2026 is as likely a slip as a date from last
       cycle, and it went in silently and came out on the finder as 2027. */
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const gone = (body.intakes || []).map((i, n) => ({ i, n: n + 1 }))
      .filter(x => x.i.deadline && new Date(x.i.deadline) < today);
    if (gone.length && !PAST_OK) {
      const say = d => new Date(d).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' });
      const next = d => { const t = new Date(d); t.setFullYear(today.getFullYear() + (new Date(today.getFullYear(), t.getMonth(), t.getDate()) < today ? 1 : 0)); return say(t); };
      $('#pmErr').innerHTML = 'Not saved yet. ' + gone.map(x => 'Intake ' + x.n + ' closes <b>' + say(x.i.deadline)
        + '</b>, which has passed — the site would show it as <b>' + next(x.i.deadline) + '</b>.').join(' ')
        + ' Is that right? <button type="button" class="btn btn-sm btn-primary" id="pmPastOk" style="margin-left:8px">Yes, save it</button>'
        + ' <button type="button" class="btn btn-sm btn-ghost" id="pmPastNo">No, let me fix the date</button>';
      $('#pmErr').style.display = 'block';
      $('#pmErr').scrollIntoView({ block: 'nearest' });
      return;
    }
    PAST_OK = false;
    REQ_FIELDS.forEach(([key]) => { const el = $('#fReq_' + key); if (el) { el.removeAttribute('aria-invalid'); el.style.borderColor = ''; } });
    try {
      ...
      $('#pmErr').textContent = err.message;
      $('#pmErr').style.display = 'block';
      /* The server names the box it refused — "reqs.ieltsMin" — so the red
         border lands on that box, not on the message alone. */
      (err.fields || []).forEach((f, n) => {
        const el = f && /^reqs\./.test(f.field) ? $('#fReq_' + f.field.slice(5)) : null;
        if (el) { el.setAttribute('aria-invalid', 'true'); el.style.borderColor = '#c0392b'; if (n === 0) el.focus(); }
      });
      $('#pmErr').scrollIntoView({ block: 'nearest' });
    }
    return;
