...
let UNIS = [], uniOpen = null, uniDirty = false;

async function loadUnis() {
  const r = await api('GET', '/api/staff/universities');
  UNIS = r.universities;
  $('#nPages').textContent = UNIS.length;
  paintUnis();
}

function paintUnis() {
  const q = ($('#uq').value || '').trim().toLowerCase();
  const rows = UNIS.filter(u => !q || (u.name + ' ' + u.shortName + ' ' + u.city).toLowerCase().includes(q));
...
document.addEventListener('input', e => {
  if (e.target && e.target.id === 'uq') paintUnis();
});
