...
BODY = """
    ...
    <div class="p-sec">
      <div class="p-sec-head"><h2>The work itself</h2>
        <select id="tkWho" style="margin-left:auto;padding:8px 11px;
          font:400 12.8px/1.4 var(--sans);border:1.5px solid #d8dde4;border-radius:9px">
          <option value="">Everybody</option></select>
        <select id="tkState" style="padding:8px 11px;font:400 12.8px/1.4 var(--sans);
...
function paintTasks() {
  ...
  $('#tkPeople').innerHTML = people.length ? people.map(p =>
    '<tr' + (p.late ? ' class="late"' : '') + '>' +
      ...
      '<td style="text-align:right"><button type="button" class="lnk" data-tkwho="' +
        (p.id == null ? 'none' : p.id) + '">see their work</button></td>' +
    '</tr>').join('')
...
document.addEventListener('click', async e => {
  ...
  /* "Admin should be able to send message to the counsellors who is working
     on file saying there is this much delay or task has to be completed by so
     and so time." The note is drafted for them with the delay already in it,
     because a chase that takes four sentences to write is a chase nobody
     sends. It goes to the counsellor's screen and, if they are not on it,
     to their phone and their inbox. */
  const c = e.target.closest('[data-tkchase]');
  if (c) {
    const over = c.dataset.tkover === '' ? null : Number(c.dataset.tkover);
    const draft = over != null && over >= 0
      ? '"' + c.dataset.tktitle + '" is ' + over + ' day' + (over === 1 ? '' : 's')
        + ' past the date we agreed. Please finish it or tell me what is holding it up.'
      : '"' + c.dataset.tktitle + '" is coming up. Where are we with it?';
    const said = prompt('What do you want to say to the counsellor?', draft);
    if (said == null || !said.trim()) return;
    try {
      await api('POST', '/api/staff/student/' + c.dataset.tkchase + '/guide', { body: said.trim() });
      toast('Sent. It is on their screen, and in their inbox if they are not looking.');
    } catch (err) { toast(err.message || 'That did not send.'); }
  }
});
