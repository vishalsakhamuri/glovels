...
BODY = """
    <style>
      ...
      /* A heading with controls beside it has to be allowed to fold, or the
         controls push the whole page sideways on a phone — which is exactly
         what the two Tasks filters did at 390px. */
      .p-sec-head{flex-wrap:wrap}
      /* The funnel. One tile per phase, wrapping rather than scrolling, with
         the count large enough to read across a desk. */
      .fn{display:flex;gap:8px;flex-wrap:wrap}
      .fn button{flex:1 1 118px;min-width:0;text-align:left;cursor:pointer;
        padding:11px 12px;border-radius:11px;background:var(--paper);
        border:1px solid var(--line,#e6e9ee);font:inherit}
      .fn button:hover{border-color:#c3ccd8}
      .fn button[aria-pressed="true"]{border-color:#1a4fb4;box-shadow:0 0 0 2px rgba(26,79,180,.14)}
      .fn b{display:block;font:800 20px/1 var(--sans);color:var(--navy-900,#0b1e31)}
      .fn span{display:block;font:600 11.2px/1.35 var(--sans);color:var(--muted);margin-top:4px}
      .fn i{display:block;font-style:normal;font:700 10.8px/1.4 var(--sans);margin-top:5px}
      .fn .bad{color:#a8343a}
      .fn .wait{color:#8a6d1f}
      .fn button.none b,.fn button.none span{opacity:.42}
      @media (max-width:430px){ .fn button{flex:1 1 calc(50% - 4px)} }
      .p-sec-head select,.p-sec-head input{min-width:0}
    ...
    <!-- The standard list, and how long each step is allowed to take.
         Two kinds of rule and the screen says which is which: a number of
         days from the day the file opened, or a number of days BEFORE the
         university closes. Mixing them up is the one mistake that produces a
         board full of dates nobody understands. -->
    <!-- Asking a counsellor about a file.
         Three ready-made questions because the office asks the same three,
         and a question typed from scratch every time is a question that
         often does not get asked at all. "Expect an answer" is the whole
         difference between this and the old one-way note: it keeps the line
         on the board until somebody replies. -->
    <div class="modal" id="askModal" role="dialog" aria-modal="true">
      <div class="sheet" style="width:min(560px,100%)">
        <button class="sheet-close" data-aclose aria-label="Close">✕</button>
        <h3 id="askTitle">Ask the counsellor</h3>
        <p class="lead" id="askAbout"></p>
        <div id="askCanned" style="display:flex;flex-wrap:wrap;gap:7px;margin-bottom:11px"></div>
        <textarea id="askBody" rows="4" placeholder="What do you want to ask?"
          style="width:100%;padding:11px 12px;font:400 13.4px/1.55 var(--sans);
          border:1.5px solid #d8dde4;border-radius:11px;resize:vertical"></textarea>
        <label style="display:flex;align-items:center;gap:8px;margin-top:11px;
          font:600 12.6px/1.5 var(--sans);color:var(--navy-800)">
          <input type="checkbox" id="askExpect" checked>
          Expect an answer &mdash; keep it on my list until they reply</label>
        <p id="askErr" style="display:none;margin:12px 0 0;padding:11px 13px;border-radius:10px;
          background:#fdeceb;border:1px solid #f3c9c6;font:600 12.6px/1.55 var(--sans);
          color:#8d2f2a"></p>
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:16px">
          <button type="button" class="btn ghost" data-aclose>Cancel</button>
          <button type="button" class="btn" id="askSend">Send</button>
        </div>
      </div>
    </div>

    <!-- What one counsellor actually did. Opened from their row, so the
         office reads the record before asking about it. -->
    <div class="modal" id="weekModal" role="dialog" aria-modal="true">
      <div class="sheet" style="width:min(640px,100%)">
        <button class="sheet-close" data-wclose aria-label="Close">✕</button>
        <h3 id="weekWho">Their week</h3>
        <div style="display:flex;gap:7px;margin-bottom:14px">
          <button type="button" class="btn ghost small" data-week="7">Last 7 days</button>
          <button type="button" class="btn ghost small" data-week="30">Last 30 days</button>
          <button type="button" class="btn ghost small" data-week="90">Last 90 days</button>
        </div>
        <div id="weekBody"></div>
      </div>
    </div>

    <div class="modal" id="ruleModal" role="dialog" aria-modal="true">
      <div class="sheet" style="width:min(680px,100%)">
        <button class="sheet-close" data-rclose aria-label="Close">✕</button>
        ...
        <p id="rkErr" style="display:none;margin:12px 0 0;padding:11px 13px;border-radius:10px;
          ...
          color:#8d2f2a"></p>
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:16px">
          <button type="button" class="btn ghost" data-rclose>Cancel</button>
          <button type="button" class="btn" id="rkSave">Save the list</button>
        </div>
      </div>
    </div>
      ...
      <div class="sheet" style="width:min(520px,100%)">
        ...
        <div id="pForm">
          ...
          <div class="field" style="margin-bottom:10px"><label for="pPhone">Mobile (optional)</label>
            <input id="pPhone" inputmode="tel" placeholder="98765 43210" pattern="[+\d\s().-]*" maxlength="20"></div>
    ...
    <div class="otabs" role="tablist">
      ...
      <button class="otab" role="tab" data-o="email" aria-selected="false">
        Email</button>
      <button class="otab" role="tab" data-o="tasks" aria-selected="false">
        Tasks <span class="n">0</span></button>
      <button class="otab" role="tab" data-o="chats" aria-selected="false">
        Conversations <span class="n">0</span></button>
      <button class="otab" role="tab" data-o="app" aria-selected="false">
        Mobile app</button>
      <button class="otab" role="tab" data-o="traffic" aria-selected="false">
        Traffic</button>
    </div>
    ...
    <!-- ======================================================= tasks ===
         "We need to help admin to identify, track and make sure counsellor
          completes the tasks."

         Two halves, and the order of them is the point. The scoreboard is
         first, because the question an owner opens this screen with is "who
         is behind", not "what is task 447". The list is second, and it opens
         filtered to what is late, because a board that opens showing four
         hundred rows in good order is a board nobody reads twice. -->
    <div class="opane" id="o-tasks">
    <div class="p-sec">
      <div class="p-sec-head"><h2 id="funnelCard">Where everybody has got to</h2>
        <span id="fnChip" hidden class="st wait" style="text-transform:none;
          letter-spacing:0;cursor:pointer" title="Show every phase again"></span></div>
      <div id="fnStrip" class="fn"></div>
      <p style="margin:12px 0 0;font-size:12.2px;color:var(--muted);line-height:1.6">
        A student&rsquo;s phase is worked out from their tasks &mdash; nobody sets it, so it cannot
        go stale. They move on when everything in a phase is finished; <b>Applying</b> holds one
        submission per shortlisted university, so a ten-university shortlist is ten of them.
        <b>Waiting</b> counts files where nothing is late but nothing has moved for a fortnight
        either &mdash; the ones that stop quietly.</p>
    </div>

    <div class="p-sec">
      <div class="p-sec-head"><h2 id="tasksCard">Who is on top of their files</h2>
        <span id="tkLate" hidden class="st bad" style="text-transform:none;
          letter-spacing:0"></span>
        <button type="button" id="tkRules" class="btn ghost small"
          style="margin-left:auto">Change the standard list</button></div>
      <div class="p-card" style="padding:0;overflow-x:auto">
        <table class="tbl" style="margin:0">
          <thead><tr><th>Counsellor</th>
            <th style="text-align:right">Still to do</th>
            <th style="text-align:right">Past its date</th>
            <th style="text-align:right">Finished</th>
            <th style="text-align:right">On time</th>
            <th></th></tr></thead>
          <tbody id="tkPeople"></tbody>
        </table>
      </div>
      <p style="margin:12px 0 0;font-size:12.2px;color:var(--muted);line-height:1.6">
        <b>On time</b> is the share of finished work that was finished on or before the date
        agreed with the student. It is blank until somebody has finished something &mdash; a
        counsellor three weeks into their first file has no record yet, and a percentage
        invented out of nothing is worse than no percentage.</p>
    </div>

    <div class="p-sec">
      <div class="p-sec-head"><h2>The work itself</h2>
        <select id="tkWho" style="margin-left:auto;padding:8px 11px;min-width:0;max-width:100%;
          font:400 12.8px/1.4 var(--sans);border:1.5px solid #d8dde4;border-radius:9px">
          <option value="">Everybody</option></select>
        <select id="tkState" style="padding:8px 11px;min-width:0;max-width:100%;
          font:400 12.8px/1.4 var(--sans);border:1.5px solid #d8dde4;border-radius:9px">
          <option value="late">Past its date</option>
          <option value="live" selected>Still to do</option>
          <option value="done">Finished</option>
          <option value="all">Everything</option>
        </select></div>
      <div class="p-card" style="padding:0;overflow-x:auto">
        <table class="tbl" style="margin:0">
          <thead><tr><th>Student</th><th>What</th><th>Counsellor</th>
            <th>By when</th><th>State</th><th></th></tr></thead>
          <tbody id="tkRows"></tbody>
        </table>
      </div>
      <p id="tkNone" hidden style="margin:12px 0 0;font-size:13px;color:var(--muted)"></p>
    </div>
    <div class="p-sec">
      <div class="p-sec-head"><h2 id="qCard">Questions you have asked</h2>
        <span id="qOpen" hidden class="st bad" style="text-transform:none;letter-spacing:0"></span></div>
      <div id="qList"></div>
      <p id="qNone" hidden style="margin:0;font-size:13px;color:var(--muted)"></p>
    </div>
    ...
    <div class="opane" id="o-app">
    <div class="p-sec">
      <div class="p-sec-head"><h2>Mobile app</h2></div>
      <!-- What makes the Android app an app rather than a browser wearing a
           costume. Android drops the address bar only if it can fetch
           /.well-known/assetlinks.json from this domain and find the signing
           certificate of the app asking. Get it wrong and nothing fails: the
           app installs, opens, works, and carries a URL bar across the top of
           every screen with nothing saying why. -->
      <div class="p-card" id="appCard">
        <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
          <h3 style="margin:0;font-size:15px">Android</h3>
          <span class="st" id="appMode">checking&hellip;</span>
          <span style="flex:1"></span>
          <a class="btn btn-ghost btn-sm" href="/.well-known/assetlinks.json"
            target="_blank" rel="noopener">See what Google is served</a>
        </div>
        <p style="margin:10px 0 0;font-size:12.6px;line-height:1.65;color:var(--navy-800)"
          id="appSays"></p>

        <div class="two" style="margin-top:14px">
          <div class="field"><label for="aPkg">Package name</label>
            <input id="aPkg" placeholder="com.glovels.app" autocomplete="off"></div>
          <div class="field"><label for="aPrints">SHA-256 certificate fingerprint</label>
            <input id="aPrints" autocomplete="off"
              placeholder="AB:CD:EF:&hellip; (paste one per line, or several)"></div>
        </div>
        <p style="margin:-2px 0 12px;font-size:11.8px;color:var(--muted);line-height:1.6">
          Both come from the Play Console once the first build has been uploaded:
          <b>Test and release &rarr; Setup &rarr; App signing</b>. Copy the
          <b>SHA-256</b> one &mdash; the SHA-1 above it on the same page is shorter and
          will not work. Paste both the app signing and the upload certificate while you
          are testing; separate them with a space or a comma. Saved here and in force
          immediately, with no redeploy.</p>
        <p class="ferr" id="appErr" hidden></p>
        <button type="button" class="btn btn-primary btn-sm" id="appSave">Save</button>
      </div>
    </div>
    </div><!-- /o-app -->

    <!-- How the site is found. Counted by the server from the pages it
         serves — not by a tag a blocker can strip — so the answer to "is
         ChatGPT sending us anyone, and has Claude read our pages?" is here,
         alongside Google. -->
    <div class="opane" id="o-traffic">
    <div class="p-sec">
      <div class="p-sec-head"><h2 id="trafficCard">How the site is found</h2>
        <span style="flex:1"></span>
        <div class="tr-range" role="group" aria-label="Range">
          <button type="button" class="btn btn-ghost btn-sm" data-days="7">7 days</button>
          <button type="button" class="btn btn-primary btn-sm" data-days="30">30 days</button>
          <button type="button" class="btn btn-ghost btn-sm" data-days="90">90 days</button>
        </div>
      </div>
      <style>
        .tr-range{display:flex;gap:6px}
        .tr-tiles{--tiles:6;margin:0 0 16px}
        .tr-chart{display:grid;grid-auto-flow:column;grid-auto-columns:1fr;align-items:end;
          gap:2px;height:150px;padding:6px 0 0;border-bottom:1px solid #e3e8ee}
        .tr-chart b{display:block;position:relative;background:var(--navy-600,#1f4b7a);
          border-radius:3px 3px 0 0;min-height:2px;transition:height .2s}
        .tr-chart b i{position:absolute;left:0;right:0;bottom:0;background:#6ecf8f;border-radius:3px 3px 0 0}
        .tr-chart b:hover{filter:brightness(1.15)}
        .tr-axis{display:flex;justify-content:space-between;font:600 10.5px/1 var(--sans);color:var(--muted);margin-top:6px}
        .tr-key{display:flex;gap:16px;font:600 11.5px/1 var(--sans);color:var(--muted);margin:10px 0 0}
        .tr-key i{display:inline-block;width:10px;height:10px;border-radius:2px;vertical-align:-1px;margin-right:5px}
        .tr-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px;margin-top:14px}
        .tr-list{list-style:none;margin:8px 0 0;padding:0}
        .tr-list li{display:flex;align-items:center;gap:10px;padding:6px 0;border-top:1px solid #eef1f5;font-size:13px}
        .tr-list li:first-child{border-top:0}
        .tr-list li span{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
        .tr-list li b{font-variant-numeric:tabular-nums}
        .tr-list li em{font-style:normal;font:600 10.5px/1 var(--sans);color:var(--muted);white-space:nowrap}
        .tr-list .bar{height:4px;border-radius:2px;background:var(--navy-600,#1f4b7a);opacity:.7;flex:0 0 auto}
        .tr-empty{margin:8px 0 0;font-size:12.6px;color:var(--muted);line-height:1.6}
        .tr-ai{background:#e8f5ee;color:#1d6b3a}.tr-search{background:#e9f0fb;color:#1a4fb4}.tr-other{background:#f1f3f6;color:#5b6675}
        .tr-tag{font:800 9.4px/1 var(--sans);letter-spacing:.06em;text-transform:uppercase;padding:4px 6px;border-radius:5px}
        #trBots li span{white-space:normal;line-height:1.3}
        #trBots li .bar{display:none}
      </style>

      <div class="out tiles tr-tiles">
        <div><b id="trVisitors">&mdash;</b><span>Visitors</span></div>
        <div><b id="trViews">&mdash;</b><span>Page views</span></div>
        <div><b id="trAi">&mdash;</b><span>From AI assistants</span></div>
        <div><b id="trSearch">&mdash;</b><span>From search</span></div>
        <div><b id="trEnq">&mdash;</b><span>Enquiries</span></div>
        <div><b id="trCrawls">&mdash;</b><span>AI crawler reads</span></div>
      </div>

      <div class="p-card" style="margin-bottom:0">
        <h3 style="margin:0 0 4px;font-size:15px">Visitors a day</h3>
        <p style="margin:0 0 10px;font-size:12.2px;color:var(--muted)">People, not page views &mdash; one person on one day counts once.
          The green part is those who came from ChatGPT, Claude, Perplexity, Gemini or Copilot.</p>
        <div class="tr-chart" id="trChart"></div>
        <div class="tr-axis"><span id="trFrom"></span><span id="trTo"></span></div>
        <div class="tr-key"><span><i style="background:var(--navy-600,#1f4b7a)"></i>Visitors</span><span><i style="background:#6ecf8f"></i>From an AI assistant</span></div>
      </div>

      <div class="tr-grid">
        <div class="p-card"><h3 style="margin:0;font-size:15px">Where visitors come from</h3>
          <ul class="tr-list" id="trSources"></ul></div>
        <div class="p-card"><h3 style="margin:0;font-size:15px">AI assistants</h3>
          <p class="tr-empty" style="margin-top:2px">People who arrived from a ChatGPT, Claude, Perplexity, Gemini or Copilot answer.</p>
          <ul class="tr-list" id="trAiList"></ul></div>
        <div class="p-card"><h3 style="margin:0;font-size:15px">Search engines</h3>
          <ul class="tr-list" id="trSearchList"></ul></div>
        <div class="p-card"><h3 style="margin:0;font-size:15px">AI &amp; search crawlers</h3>
          <p class="tr-empty" style="margin-top:2px">Who has read the site, and when they were last here. An assistant that has never crawled a page cannot recommend it.</p>
          <ul class="tr-list" id="trBots"></ul></div>
        <div class="p-card"><h3 style="margin:0;font-size:15px">Pages the AI crawlers read most</h3>
          <ul class="tr-list" id="trAiPages"></ul></div>
        <div class="p-card"><h3 style="margin:0;font-size:15px">Most-read pages</h3>
          <ul class="tr-list" id="trPages"></ul></div>
        <div class="p-card"><h3 style="margin:0;font-size:15px">Social &amp; other sites</h3>
          <ul class="tr-list" id="trRefs"></ul></div>
        <div class="p-card"><h3 style="margin:0;font-size:15px">Campaigns</h3>
          <p class="tr-empty" style="margin-top:2px">Links carrying <code>?utm_campaign=</code> &mdash; an Instagram post, a WhatsApp broadcast, an ad.</p>
          <ul class="tr-list" id="trCampaigns"></ul></div>
        <div class="p-card"><h3 style="margin:0;font-size:15px">Enquiries, by where they came from</h3>
          <ul class="tr-list" id="trEnqBy"></ul>
          <ul class="tr-list" id="trDevices" style="margin-top:14px"></ul></div>
      </div>

      <div class="p-card" id="gaCard" style="margin-top:14px">
        <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
          <h3 style="margin:0;font-size:15px">Google Analytics</h3>
          <span class="st" id="gaMode">checking&hellip;</span>
          <span style="flex:1"></span>
          <a class="btn btn-ghost btn-sm" href="https://analytics.google.com/" target="_blank" rel="noopener">Open Google Analytics</a>
        </div>
        <p style="margin:10px 0 0;font-size:12.6px;line-height:1.65;color:var(--navy-800)">
          The numbers above are counted by this server and need nothing from Google. Google
          Analytics is worth having as well &mdash; it is what Google Ads and Search Console
          talk to, and its reports go deeper on what people do once they are here. Paste the
          <b>measurement ID</b> (Google Analytics &rarr; Admin &rarr; Data streams &rarr; your
          site; it looks like <code>G-XXXXXXXXXX</code>) and the tag is on every public page
          from the next page served. The portal and these office screens are never tagged.</p>
        <div class="two" style="margin-top:14px">
          <div class="field"><label for="gaId">Measurement ID</label>
            <input id="gaId" placeholder="G-XXXXXXXXXX" autocomplete="off" spellcheck="false"></div>
        </div>
        <p class="ferr" id="gaErr" hidden></p>
        <button type="button" class="btn btn-primary btn-sm" id="gaSave">Save</button>
      </div>
    </div>
    </div><!-- /o-traffic -->
...
function showTab(which) {
  ...
  if (which === 'traffic' && typeof trOpen === 'function') trOpen();
  if (which === 'tasks' && typeof tkOpen === 'function') tkOpen();
...
const TAB_OF = { everyStudent: 'students', everyOrder: 'orders',
  theMoney: 'money', mailCard: 'email', everyChat: 'chats', appCard: 'app',
  trafficCard: 'traffic', gaCard: 'traffic' };
function tabForHash() {
...
/* Which person this modal is editing, or null when it is creating one. The
   form, its inline errors and its phone check are the same either way — the
   only thing that changes is the verb. */
let EDITING = null;

function openPersonEdit(p) {
  EDITING = p;
  $('#pTitle').textContent = 'Edit ' + p.name;
  $('#pForm').style.display = '';
  ...
  /* The role, the permissions and the generated password are not this form's
     business when editing — they have their own controls beside the person's
     name, and offering them here would mean a save could quietly change one. */
  $('#pRole').closest('.field').style.display = 'none';
  const box = $('#pPermBox'); if (box) box.style.display = 'none';
...
function openPersonForm() {
  EDITING = null;
  $('#pRole').closest('.field').style.display = '';
  const box0 = $('#pPermBox'); if (box0) box0.style.display = '';
  $('#pSave').textContent = 'Create the account';
...
document.addEventListener('click', async e => {
  ...
  if (e.target.closest('#pSave')) {
    ...
    if (ph && !(/^\+?[\d\s().-]+$/.test(ph) && ph.replace(/\D/g, '').length >= 8 && ph.replace(/\D/g, '').length <= 15)) {
      $('#pErr').textContent = 'That is not a phone number — digits only, with the country code if it is not Indian (+49 89 2019 4090).';
    ...
    try {
      if (editing) {
        await api('PUT', '/api/staff/people/' + editing.id,
      ...
      /* Left open, with what they typed still in it. That is the whole reason
         this is a form and not a chain of prompts. */
      $('#pErr').textContent = err.message;
      $('#pErr').style.display = 'block';
    } finally {
      btn.disabled = false;
      btn.textContent = editing ? 'Save' : 'Create the account';
    }
    return;
  }
  ...
  if (ed) {
    ...
    if (!p) return;
    /* Three native prompts, one after another, and a server rejection threw
       all three answers away — the email was refused and the name and phone
       they had just retyped went with it. The Add form on this same screen is
       a modal with inline errors and has been all along; this uses it. */
    openPersonEdit(p);
    return;
  }
...
/* ------------------------------------------------------------- the Android app
 *
 * Two boxes and one consequence. Android will only drop the browser's address
 * bar from the app if it can fetch /.well-known/assetlinks.json from this
 * domain and find the app's signing certificate in it. Nothing about getting
 * this wrong looks like an error — the app installs, opens and works, wearing
 * a URL bar — so this screen says out loud whether the file is currently
 * verifying anything, rather than only whether a box has something in it.
 */
async function paintApp() {
  let a = null;
  try { a = await api('GET', '/api/staff/android'); }
  catch (e) { return; }
  $('#aPkg').value = a.package || '';
  $('#aPrints').value = (a.fingerprints || []).join('  ');
  const mode = $('#appMode');
  mode.textContent = a.live ? 'verifying' : 'not set up';
  mode.className = 'st ' + (a.live ? 'ok' : 'none');
  $('#appSays').textContent = a.live
    ? 'Android is being told that ' + a.package + ' may open this site without an '
      + 'address bar, against ' + (a.fingerprints || []).length + ' certificate'
      + ((a.fingerprints || []).length === 1 ? '' : 's') + '.'
    : 'Nothing is being served yet, which is the right answer until there is an app. '
      + 'Fill both boxes in after the first upload to the Play Console and the file '
      + 'starts working immediately.';
}

$('#appSave').addEventListener('click', async () => {
  const btn = $('#appSave'), err = $('#appErr');
  err.hidden = true;
  btn.disabled = true;
  const was = btn.textContent;
  btn.textContent = 'Saving…';
  try {
    await api('PUT', '/api/staff/android', {
      package: $('#aPkg').value,
      /* Split on the server, which is where the checking is. A box that accepts
         one shape and an endpoint that accepts another is two rules to keep in
         step, and they never stay in step. */
      fingerprints: $('#aPrints').value,
    });
    await paintApp();
    toast('Saved — Google is being served the new file now.');
  } catch (e) {
...
let TR_DAYS = 30;
const trN = n => Number(n || 0).toLocaleString('en-IN');
const trSourceName = { ai: 'AI assistants', search: 'Search engines', ads: 'Paid search',
  social: 'Social', campaign: 'Campaign links', referral: 'Other websites', direct: 'Direct / typed in' };
function trList(id, rows, opts) {
  const ul = $('#' + id);
  opts = opts || {};
  if (!rows || !rows.length) {
    ul.innerHTML = opts.empty === '' ? '' : '<li class="tr-empty" style="border:0;display:block">' + esc(opts.empty || 'Nothing yet.') + '</li>';
    return;
  }
  const max = Math.max(...rows.map(r => r.count || r.hits || 0), 1);
  ul.innerHTML = rows.map(r => {
    const n = r.count != null ? r.count : r.hits;
    const label = opts.label ? opts.label(r) : r.name;
    const extra = opts.extra ? opts.extra(r) : '';
    return '<li><span title="' + esc(label) + '">' + esc(label) + '</span>' + extra
      + '<i class="bar" style="width:' + Math.max(4, Math.round(60 * n / max)) + 'px"></i><b>' + trN(n) + '</b></li>';
  }).join('');
}
const trAgo = iso => {
  if (!iso) return '';
...
const trDay = day => new Date(day + 'T00:00:00').toLocaleDateString('en-GB', { day: 'numeric', month: 'short' });
async function paintTraffic() {
  let t;
  try { t = await api('GET', '/api/staff/traffic?days=' + TR_DAYS); }
  catch (e) { $('#trSources').innerHTML = '<li class="tr-empty">' + esc(e.message) + '</li>'; return; }
  $('#trVisitors').textContent = trN(t.totals.visitors);
  $('#trViews').textContent = trN(t.totals.views);
  $('#trAi').textContent = trN(t.totals.ai);
  $('#trSearch').textContent = trN(t.totals.search);
  $('#trEnq').textContent = trN(t.totals.enquiries);
  $('#trCrawls').textContent = trN(t.totals.aiCrawls);

  const max = Math.max(...t.byDay.map(d => d.visitors), 1);
  $('#trChart').innerHTML = t.byDay.map(d => {
    const h = Math.round(100 * d.visitors / max);
    const ai = d.views ? Math.round(100 * d.ai / d.views) : 0;
    return '<b style="height:' + h + '%" title="' + esc(trDay(d.day) + ': ' + trN(d.visitors) + ' visitors, ' + trN(d.views)
      + ' page views' + (d.ai ? ', ' + trN(d.ai) + ' from AI assistants' : '') + (d.enquiries ? ', ' + trN(d.enquiries) + ' enquiries' : '')
      + (d.bots ? ', ' + trN(d.bots) + ' crawler visits' : '')) + '">'
      + (ai ? '<i style="height:' + Math.min(100, ai) + '%"></i>' : '') + '</b>';
  }).join('');
  $('#trFrom').textContent = trDay(t.byDay[0].day);
  $('#trTo').textContent = 'today';

  trList('trSources', t.sources, { label: r => trSourceName[r.name] || r.name, empty: 'No visits counted yet — this starts the moment the site is live.' });
  trList('trAiList', t.ai, { empty: 'Nobody from an AI assistant yet. The crawler list says whether they have read the site at all.' });
  trList('trSearchList', t.search, { empty: 'Nobody from a search engine yet.' });
  trList('trBots', t.bots, {
    empty: 'No crawler has been here yet. Googlebot usually arrives within a day of the domain going live; the AI readers within a week or two.',
    extra: r => '<em class="tr-tag tr-' + (r.group === 'ai' ? 'ai' : r.group === 'search' ? 'search' : 'other') + '">'
      + (r.group === 'ai' ? 'AI' : r.group === 'search' ? 'search' : 'other') + '</em><em>' + esc(r.pages + ' pages · ' + trAgo(r.last)) + '</em>',
  });
  trList('trAiPages', t.aiPages, { empty: 'Nothing read by an AI crawler yet.' });
  trList('trPages', t.pages, { empty: 'No page views yet.' });
  trList('trRefs', (t.social || []).concat(t.referrals || []), { empty: 'No links from other sites yet.' });
  trList('trCampaigns', t.campaigns, { empty: 'No campaign links yet. Add ?utm_campaign=name&utm_source=instagram to a link and it shows here.' });
  trList('trEnqBy', t.enquiriesBy, { empty: 'No enquiries in this period.' });
  trList('trDevices', t.devices, { label: r => ({ phone: 'On a phone', desktop: 'On a computer', tablet: 'On a tablet' })[r.name] || r.name, empty: '' });
}
$$('.tr-range button').forEach(b => b.addEventListener('click', () => {
  TR_DAYS = Number(b.dataset.days) || 30;
  $$('.tr-range button').forEach(x => { x.className = 'btn btn-sm ' + (x === b ? 'btn-primary' : 'btn-ghost'); });
  paintTraffic();
}));

async function paintGa() {
  try {
    const a = await api('GET', '/api/staff/analytics');
    $('#gaId').value = a.gaId || '';
    const m = $('#gaMode');
    m.textContent = a.gaId ? 'tag on every public page' : 'not connected';
    m.className = 'st ' + (a.gaId ? 'ok' : 'wait');
  } catch (e) { $('#gaMode').textContent = 'could not read'; }
}
$('#gaSave').addEventListener('click', async () => {
  const btn = $('#gaSave'), err = $('#gaErr');
  err.hidden = true; btn.disabled = true;
  const was = btn.textContent; btn.textContent = 'Saving…';
  try {
    const r = await api('PUT', '/api/staff/analytics', { gaId: $('#gaId').value });
    await paintGa();
    toast(r.gaId ? 'Saved — the Google tag is on every public page from now.' : 'Saved — the Google tag is off.');
  } catch (e) {
    err.textContent = e.message || 'That did not save.'; err.hidden = false;
  } finally { btn.disabled = false; btn.textContent = was; }
});

/* Painted when the tab is opened rather than on every visit to this screen —
   it reads every visit of the period. */
let trPainted = false;
function trOpen() { if (!trPainted) { trPainted = true; paintTraffic(); paintGa(); } }
if ($('#o-traffic') && $('#o-traffic').classList.contains('active')) trOpen();

/* ==================================================================== tasks
 *
 * "How can the admin make sure all the tasks are completed and we are good."
 *
 * The scoreboard answers that in one screen; the table underneath is how you
 * act on it. Read on opening the tab rather than on every visit to the
 * screen, like Traffic: it walks every task on the books.
 */
let TK = { tasks: [], people: [], counts: {} };
let tkPainted = false;
...
function tkWhen(t) {
  ...
  return '<b style="' + (late ? 'color:#a8343a' : '') + '">' + esc(t.due) + '</b>' +
    ...
    /* Where the date came from. An unexplained date is a date people argue
       with instead of meeting. */
    '<span style="display:block;font-size:11px;color:var(--muted)">' +
      (t.basis === 'deadline' ? 'before the university closes'
        : t.basis === 'set' ? 'set by hand' : 'from the standard list') + '</span>';
}

const TK_STATE = { open: ['wait', 'Not started'], doing: ['wait', 'In hand'],
  done: ['ok', 'Finished'], blocked: ['bad', 'Stuck'], dropped: ['wait', 'Does not apply'] };

function paintTasks() {
  /* ---- the funnel ----
     One tile per phase the office actually has somebody in. A phase with
     nobody in it is drawn faded rather than hidden, because a gap in the
     middle of a journey is itself worth seeing — nobody at Writing while
     twelve sit at Shortlisting says something. */
  const fn = TK.funnel || [];
  $('#fnStrip').innerHTML = fn.map(f =>
    '<button type="button" class="' + (f.students ? '' : 'none') + '" data-fn="' + f.key + '"' +
      ' aria-pressed="' + (TK_PHASE === f.key) + '">' +
      '<b>' + f.students + '</b><span>' + esc(f.label) + '</span>' +
      (f.late ? '<i class="bad">' + f.late + ' behind</i>'
        : f.waiting ? '<i class="wait">' + f.waiting + ' waiting</i>' : '<i>&nbsp;</i>') +
    '</button>').join('');
  const fnChip = $('#fnChip');
  if (fnChip) {
    const picked = fn.find(f => f.key === TK_PHASE);
    fnChip.hidden = !picked;
    fnChip.textContent = picked ? 'Showing ' + picked.label + ' — show everybody' : '';
  }

  const people = TK.people || [];
  $('#tkPeople').innerHTML = people.length ? people.map(p =>
    '<tr' + (p.late ? ' class="late"' : '') + '>' +
      '<td><b>' + esc(p.name || 'Nobody') + '</b></td>' +
      '<td style="text-align:right">' + p.open + '</td>' +
      '<td style="text-align:right' + (p.late ? ';color:#a8343a;font-weight:800' : '') + '">'
        + p.late + '</td>' +
      '<td style="text-align:right">' + p.done + '</td>' +
      '<td style="text-align:right">' + (p.percent == null ? '—' : p.percent + '%') + '</td>' +
      '<td style="text-align:right;white-space:nowrap"><button type="button" class="lnk" data-tkwho="' +
        (p.id == null ? 'none' : p.id) + '">their work</button>' +
        /* The record, before the conversation. "What have you done since one
           week" is a question the screen can answer without asking anybody. */
        (p.id ? ' · <button type="button" class="lnk" data-tkweek="' + p.id +
          '">their week</button>' : '') + '</td>' +
    '</tr>').join('')
    : '<tr><td colspan="6" style="color:var(--muted)">Nothing on the books yet.</td></tr>';

  const chip = $('#tkLate');
  const lateN = (TK.counts || {}).late || 0;
  if (chip) {
    chip.hidden = !lateN;
    chip.textContent = lateN + ' past the agreed date';
  }
  const tab = $('.otab[data-o="tasks"] .n');
  if (tab) tab.textContent = lateN || ((TK.counts || {}).open || 0);
  ...
  $('#tkRows').innerHTML = rows.map(t => {
    ...
    return '<tr' + (t.over != null && t.over > 0 ? ' class="late"' : '') + '>' +
      ...
      '<td>' + esc(t.title) +
        (t.note ? '<span style="display:block;font-size:11.4px;color:var(--muted)">' +
          esc(t.note) + '</span>' : '') + '</td>' +
      '<td>' + (t.owner ? esc(t.owner) :
        '<span style="color:#a8343a;font-weight:700">nobody</span>') + '</td>' +
      '<td>' + tkWhen(t) + '</td>' +
      '<td><span class="st ' + st[0] + '" style="text-transform:none;letter-spacing:0">' +
        esc(st[1]) + '</span></td>' +
      '<td style="text-align:right;white-space:nowrap">' +
        (t.status === 'done' ? ''
          : '<button type="button" class="lnk" data-tkdone="' + t.id + '">mark done</button> · ') +
        '<button type="button" class="lnk" data-tkchase="' + t.studentId +
          '" data-tkid="' + t.id + '" data-tkstudent="' + esc(t.student) +
          '" data-tktitle="' + esc(t.title) + '" data-tkover="' + (t.over == null ? '' : t.over) +
          '">ask</button></td>' +
    '</tr>';
  }).join('');
  const none = $('#tkNone');
  if (none) {
    ...
    none.textContent = $('#tkState').value === 'late'
      ? 'Nothing is past its date. That is the screen working.'
      : 'Nothing here.';
  }
}
...
async function loadTasks() {
  const who = $('#tkWho').value, state = $('#tkState').value;
  const q = '?state=' + encodeURIComponent(state) + (who ? '&who=' + encodeURIComponent(who) : '')
    + (TK_PHASE ? '&phase=' + encodeURIComponent(TK_PHASE) : '');
  try { TK = await api('GET', '/api/staff/tasks' + q); }
  catch (e) { TK = { tasks: [], people: [], counts: {} }; }
  /* The people list is filled from the scoreboard, so it names exactly the
     people who have work — not every account that has ever existed. */
  const sel = $('#tkWho');
  if (sel && sel.options.length <= 1 && (TK.people || []).length) {
    sel.innerHTML = '<option value="">Everybody</option>' +
      TK.people.map(p => '<option value="' + (p.id == null ? 'none' : p.id) + '">' +
        esc(p.name || 'Nobody') + '</option>').join('');
  }
  paintTasks();
  loadQuestions();
}

$('#tkWho').addEventListener('change', loadTasks);
$('#tkState').addEventListener('change', loadTasks);

document.addEventListener('click', async e => {
  /* Clicking a phase shows the work on those files, and clicking it again —
     or the chip — puts everybody back. A filter with no way out reads as a
     screen that has broken. */
  const f = e.target.closest('[data-fn]');
  if (f) {
    TK_PHASE = (TK_PHASE === f.dataset.fn) ? '' : f.dataset.fn;
    $('#tkState').value = 'all';
    loadTasks();
    return;
  }
  if (e.target.closest('#fnChip')) { TK_PHASE = ''; loadTasks(); return; }

  const w = e.target.closest('[data-tkwho]');
  if (w) { $('#tkWho').value = w.dataset.tkwho; $('#tkState').value = 'live'; loadTasks(); return; }

  const d = e.target.closest('[data-tkdone]');
  if (d) {
    d.disabled = true;
    try { await api('PUT', '/api/staff/task/' + d.dataset.tkdone, { status: 'done' }); await loadTasks(); }
    catch (err) { toast(err.message || 'That did not save.'); d.disabled = false; }
    return;
  }

  /* "Admin should be able to monitor the student process and ask questions to
     counsellor — why this has not happened, why this much delay, what you
     have done since one week, what tasks you have completed."
     The last of those the screen answers itself, from the record, on the
     counsellor's own row. The rest open this, which drafts the question with
     the delay already in it — a chase that takes four sentences to write is
     a chase nobody sends. */
  const c = e.target.closest('[data-tkchase]');
  if (c) {
    askOpen({
      studentId: c.dataset.tkchase,
      taskId: c.dataset.tkid || '',
      title: c.dataset.tktitle || '',
      student: c.dataset.tkstudent || '',
      over: c.dataset.tkover === '' ? null : Number(c.dataset.tkover),
    });
  }
});
...
let ASK = null;
function askOpen(ctx) {
  ASK = ctx;
  const over = ctx.over;
  /* Past the date means PAST it. A task due today is due today, and telling
     a counsellor it is "0 days past the agreed date" is the screen picking a
     fight over work that is not yet late. */
  const late = over != null && over > 0;
  const dueToday = over === 0;
  $('#askTitle').textContent = 'Ask about ' + (ctx.student || 'this file');
  $('#askAbout').textContent = ctx.title
    ? (late ? '“' + ctx.title + '” — ' + over + ' day' + (over === 1 ? '' : 's')
        + ' past the agreed date.'
      : dueToday ? '“' + ctx.title + '” — due today.'
      : '“' + ctx.title + '” — due ' + (over == null ? 'soon' : 'in ' + (-over) + ' days') + '.')
    : 'A question about this file.';
  /* The three the office actually asks. Written out in full so the
     counsellor reads a question, not a prompt someone did not finish. */
  const canned = [
    ['Why has this not happened?',
     'Why has this not happened yet?' + (ctx.title ? ' (' + ctx.title + ')' : '')],
    ['Why the delay?',
     late ? 'This is ' + over + ' day' + (over === 1 ? '' : 's') + ' past the date we agreed with the '
       + 'student. What is causing the delay, and when will it be done?'
       : dueToday ? 'This is due today. Is it going out, or is something holding it up?'
       : 'What is holding this up, and when will it be done?'],
    ['When will it be done?',
     'Give me a date you can commit to for this, and tell me what you need from me to hit it.'],
    ['What have you completed this week?',
     'Tell me what you finished this week across your files, and what is slipping.'],
  ];
  $('#askCanned').innerHTML = canned.map((q, i) =>
    '<button type="button" class="btn ghost small" data-canq="' + i + '">' + esc(q[0]) + '</button>').join('');
  $('#askCanned').__q = canned;
  $('#askBody').value = late && ctx.title ? canned[1][1] : '';
  $('#askExpect').checked = true;
  $('#askErr').style.display = 'none';
  $('#askModal').classList.add('on');
  $('#askBody').focus();
}
document.addEventListener('click', e => {
  const q = e.target.closest('[data-canq]');
  if (q) {
    const list = $('#askCanned').__q || [];
    $('#askBody').value = (list[Number(q.dataset.canq)] || [])[1] || '';
    $('#askBody').focus();
    return;
  }
  if (e.target.closest('[data-aclose]') || e.target === $('#askModal')) {
    $('#askModal').classList.remove('on');
  }
  if (e.target.closest('[data-wclose]') || e.target === $('#weekModal')) {
    $('#weekModal').classList.remove('on');
  }
});
$('#askSend').addEventListener('click', async () => {
  const body = $('#askBody').value.trim();
  if (!body) { $('#askErr').textContent = 'Write the question first.'; $('#askErr').style.display = ''; return; }
  const btn = $('#askSend'), was = btn.textContent;
  btn.disabled = true; btn.textContent = 'Sending…';
  try {
    await api('POST', '/api/staff/student/' + ASK.studentId + '/guide', {
      body,
      kind: $('#askExpect').checked ? 'question' : 'note',
      taskId: ASK.taskId || undefined,
    });
    $('#askModal').classList.remove('on');
    toast($('#askExpect').checked
      ? 'Asked. It stays on your list until they answer.'
      : 'Sent. It is on their screen, and in their inbox if they are not looking.');
    loadQuestions();
  } catch (err) {
    $('#askErr').textContent = err.message || 'That did not send.';
    $('#askErr').style.display = '';
  } finally { btn.disabled = false; btn.textContent = was; }
});

async function loadQuestions() {
  let q;
  try { q = await api('GET', '/api/staff/questions'); } catch (e) { return; }
  const chip = $('#qOpen');
  if (chip) { chip.hidden = !q.open.length; chip.textContent = q.open.length + ' waiting for an answer'; }
  const card = (x, open) =>
    '<div class="p-card" style="margin-bottom:10px;padding:14px 16px' +
      (open && x.waitingDays >= 2 ? ';border-left:3px solid #a8343a' : '') + '">' +
      '<div style="display:flex;gap:10px;align-items:baseline;flex-wrap:wrap">' +
        '<a class="lnk" href="counsellor.html?student=' + x.studentId + '"><b>' +
          esc(x.student) + '</b></a>' +
        '<span style="font-size:12px;color:var(--muted)">asked ' + esc(x.to || 'the counsellor') +
          (x.about ? ' · about “' + esc(x.about) + '”' : '') + '</span>' +
        '<span style="margin-left:auto;font:700 11.6px/1.4 var(--sans);color:' +
          (open ? (x.waitingDays >= 2 ? '#a8343a' : 'var(--muted)') : '#1a7f4b') + '">' +
          (open
            ? (x.waitingDays === 0 ? 'asked today' : 'waiting ' + x.waitingDays + ' day' +
               (x.waitingDays === 1 ? '' : 's'))
            : 'answered') + '</span>' +
      '</div>' +
      '<p style="margin:9px 0 0;font-size:13.2px;line-height:1.6">' + esc(x.body) + '</p>' +
      (x.answers || []).map(a =>
        '<div style="margin:11px 0 0;padding:11px 13px;background:#f2f7f3;' +
          'border-left:3px solid #1a7f4b;border-radius:0 9px 9px 0">' +
          '<b style="font:700 11.6px/1.4 var(--sans);color:#1a7f4b">' + esc(a.from) + ' replied</b>' +
          '<p style="margin:5px 0 0;font-size:13px;line-height:1.6">' + esc(a.body) + '</p></div>').join('') +
      /* Carrying the task and its lateness, so asking again is the same
         question about the same thing rather than a fresh one with no
         context — which is how a follow-up ends up unlinked and undated. */
      (open ? '<p style="margin:10px 0 0"><button type="button" class="lnk" data-tkchase="' +
        x.studentId + '" data-tkid="' + (x.taskId || '') + '" data-tkstudent="' +
        esc(x.student) + '" data-tktitle="' + esc(x.about || '') +
        '" data-tkover="' + (x.over == null ? '' : x.over) + '">ask again</button></p>' : '') +
    '</div>';
  $('#qList').innerHTML = q.open.map(x => card(x, true)).join('')
    + q.answered.slice(0, 8).map(x => card(x, false)).join('');
  const none = $('#qNone');
  if (none) {
    none.hidden = !!(q.open.length || q.answered.length);
    none.textContent = 'You have not asked anybody anything yet. Use “ask” on a late task above.';
  }
}

/* ---- what one counsellor actually did ---- */
let WEEK_ID = null;
/* Two clicks on 90 then 7 are two requests, and whichever answers last wins
   rather than whichever was asked last. The token makes the stale one drop
   its own reply. */
let WEEK_SEQ = 0;
async function weekOpen(id, days) {
  WEEK_ID = id;
  const mine = ++WEEK_SEQ;
  $('#weekBody').innerHTML = '<p style="color:var(--muted)">Reading…</p>';
  $('#weekModal').classList.add('on');
  let r;
  try { r = await api('GET', '/api/staff/counsellor/' + id + '/activity?days=' + (days || 7)); }
  catch (e) {
    if (mine !== WEEK_SEQ) return;
    $('#weekBody').innerHTML = '<p style="color:#a8343a">' + esc(e.message) + '</p>'; return;
  }
  if (mine !== WEEK_SEQ) return;
  const a = r.activity;
  $('#weekWho').textContent = r.person.name + ' — the last ' + a.days + ' days';
  const tile = (n, label, colour) =>
    '<div style="flex:1;min-width:104px;padding:12px 13px;border-radius:11px;background:var(--paper);' +
      'border:1px solid var(--line,#e6e9ee)"><b style="display:block;font:800 21px/1 var(--sans);color:' +
      (colour || 'var(--navy-900,#0b1e31)') + '">' + n + '</b>' +
      '<span style="font:600 11.4px/1.4 var(--sans);color:var(--muted)">' + label + '</span></div>';
  $('#weekBody').innerHTML =
    '<div style="display:flex;gap:9px;flex-wrap:wrap;margin-bottom:16px">' +
      tile(a.finished, 'finished') +
      tile(a.finishedOnTime, 'on time', a.finished && a.finishedOnTime < a.finished ? '#8a6d1f' : '#1a7f4b') +
      tile(a.stillOwed, 'still owed') +
      tile(a.late, 'past its date', a.late ? '#a8343a' : undefined) +
      tile(a.students, 'files') +
    '</div>' +
    (a.finishedList.length
      ? '<h4 style="margin:0 0 8px;font:700 13px/1.4 var(--sans)">What they finished</h4><ul style="margin:0 0 16px;' +
        'padding-left:18px;font:400 13px/1.7 var(--sans)">' +
        a.finishedList.map(x => '<li>' + esc(x.title) + ' <span style="color:' +
          (x.late ? '#8a6d1f' : 'var(--muted)') + '">(' + esc(x.on) +
          (x.late ? ', late' : '') + ')</span></li>').join('') + '</ul>'
      : '<p style="margin:0 0 16px;font-size:13px;color:var(--muted)">Nothing was finished in this period.</p>') +
    (a.stuckList.length
      ? '<h4 style="margin:0 0 8px;font:700 13px/1.4 var(--sans)">What they say is stuck</h4><ul style="margin:0 0 16px;' +
        'padding-left:18px;font:400 13px/1.7 var(--sans)">' +
        a.stuckList.map(x => '<li>' + esc(x.title) +
          (x.why ? ' <span style="color:var(--muted)">— ' + esc(x.why) + '</span>' : '') + '</li>').join('') + '</ul>'
      : '') +
    (a.worst.length
      ? '<h4 style="margin:0 0 8px;font:700 13px/1.4 var(--sans)">Worth asking about</h4>' +
        a.worst.map(x => '<div style="display:flex;gap:10px;align-items:baseline;padding:8px 0;' +
          'border-top:1px solid var(--line,#e6e9ee)"><span style="flex:1;font-size:13px">' +
          esc(x.title) + '</span><span style="font:700 12px/1.4 var(--sans);color:#a8343a">' +
          x.over + 'd late</span><button type="button" class="lnk" data-tkchase="' + x.studentId +
          '" data-tkid="' + x.id + '" data-tktitle="' + esc(x.title) + '" data-tkover="' + x.over +
          '">ask</button></div>').join('')
      : '<p style="margin:0;font-size:13px;color:var(--muted)">Nothing of theirs is past its date.</p>');
}
document.addEventListener('click', e => {
  const w = e.target.closest('[data-week]');
  if (w && WEEK_ID) weekOpen(WEEK_ID, Number(w.dataset.week));
  const s = e.target.closest('[data-tkweek]');
  if (s) weekOpen(Number(s.dataset.tkweek), 7);
});

/* Escape closes the sheet you are looking at, and the page underneath holds
   still while one is open. Both of these are written over EVERY .modal on
   this screen rather than the three added with the task board: a rule that
   covers half the sheets on a page is worse than no rule, because the half
   it misses now reads as broken rather than as ordinary.
 *
 * Escape closes the TOPMOST sheet, not all of them. Nothing here opens two
 * at once today, but "closes everything" is the behaviour that quietly
 * destroys a nested sheet the first time somebody adds one.
 *
 * The lock remembers what it replaced instead of assuming the page had no
 * opinion, so a future drawer that locks scrolling for its own reasons does
 * not have its lock wiped the next time a sheet closes here. */
(function () {
  const sheets = () => $$('.modal');
  const open = () => sheets().filter(m => m.classList.contains('on'));
  let held = null;                      // what body.style.overflow was before we took it

  function sync() {
    const any = open().length > 0;
    if (any && held === null) {
      held = document.body.style.overflow;
      document.body.style.overflow = 'hidden';
    } else if (!any && held !== null) {
  ...
  document.addEventListener('keydown', e => {
    ...
    /* Last one in document order that is open — which is the one on top,
       since they are all siblings painted in order. */
    const stack = open();
    if (stack.length) stack[stack.length - 1].classList.remove('on');
  });

  const obs = new MutationObserver(sync);
  sheets().forEach(m => obs.observe(m, { attributes: true, attributeFilter: ['class'] }));
  sync();
})();
...
let RK = [];
function rkSaid(r) {
  return r.due.sla != null
    ? 'days after the file opens'
    : r.due.before >= 0 ? 'days before the university closes'
      : 'days after the university closes';
}
function paintRules() {
  $('#rkRows').innerHTML = RK.map(r =>
    '<div style="display:flex;gap:12px;align-items:center;padding:11px 0;' +
      'border-top:1px solid var(--line,#e6e9ee)">' +
      '<label style="display:flex;align-items:center;gap:7px;min-width:20px">' +
        '<input type="checkbox" data-rk-on="' + r.key + '"' + (r.off ? '' : ' checked') +
        ' aria-label="Include this step"></label>' +
      '<input data-rk-title="' + r.key + '" value="' + esc(r.title) + '" maxlength="140" ' +
        'style="flex:1;padding:8px 11px;font:400 12.8px/1.4 var(--sans);' +
        'border:1.5px solid #d8dde4;border-radius:9px">' +
      '<input type="number" data-rk-days="' + r.key + '" value="' +
        (r.due.sla != null ? r.due.sla : Math.abs(r.due.before)) + '" min="0" max="365" ' +
        'style="width:74px;padding:8px 9px;font:600 12.8px/1.4 var(--sans);' +
        'border:1.5px solid #d8dde4;border-radius:9px;text-align:right">' +
      '<span style="min-width:190px;font:400 11.8px/1.4 var(--sans);color:var(--muted)">' +
        esc(rkSaid(r)) + '</span>' +
...
document.addEventListener('click', e => {
  if (e.target.closest('[data-rclose]') || e.target === $('#ruleModal')) {
...
$('#rkSave').addEventListener('click', async () => {
  ...
  RK.forEach(r => {
    ...
    const one = { title: $('[data-rk-title="' + r.key + '"]').value.trim() };
    if (!$('[data-rk-on="' + r.key + '"]').checked) one.off = true;
    /* The sign is the screen's business, not the reader's: the box says a
       number of days and the sentence beside it says which direction. */
    if (r.due.sla != null) one.sla = days;
    else one.before = r.due.before >= 0 ? days : -days;
    out[r.key] = one;
  });
  ...
  try {
    ...
    $('#rkErr').textContent = e.message || 'That did not save.';
    $('#rkErr').style.display = '';
  } finally { btn.disabled = false; btn.textContent = was; }
});

function tkOpen() { if (!tkPainted) { tkPainted = true; loadTasks(); } }
if ($('#o-tasks') && $('#o-tasks').classList.contains('active')) tkOpen();
"""
