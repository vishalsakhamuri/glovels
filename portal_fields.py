...
const DOCS = [
  ...
  /* THE ONE SLOT THAT IS NOT A NAMED DOCUMENT.
   *
   ...
   *
   * Every other row on this list is a document we asked for by name, and that
   * is what makes the list useful — a student can read it and know what is
   * wanted. It is also what makes it a wall: a university welcome pack, an
   * accommodation contract, a scholarship form, a bank letter nobody
   * anticipated. There was nowhere to put any of them, so they went into the
   * chat, where a file is impossible to find three weeks later.
   *
   * `need:0`, and that is load-bearing. The readiness figure is "how many of
   * the documents we ASKED FOR have arrived", and a slot that can always hold
   * one more file would make that number impossible to finish. It never
   * blocks anything either, for the same reason: nobody can say in advance
   * what is in it.
   *
   * LAST on the list on purpose. It is the answer to "what about…", and a
   * catch-all offered before the named slots is a catch-all everything lands
   * in. */
  {id:'other',    name:'Other documents', need:0,
   blocks:'',
   note:'Anything else that belongs on your file and is not on this list — a '
      + 'university letter, an accommodation contract, a bank statement your '
      + 'counsellor asked for. As many files as you need. Your counsellor can '
      + 'add to it too.'}
];
