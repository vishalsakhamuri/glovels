    ...
    <section class="pane" id="pane-browse">
      <div class="p-card" style="margin-bottom:16px">
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:12px">
          ...
          <div class="field"><label>How you apply</label><select id="fType">
            <option value="">Every university</option>
            <option value="free">Fast-track — free</option>
            <option value="package">Comprehensive filing</option></select></div>
...
/* Free or charged, with the same fallback the server uses. The built-in
...
function card(p, inList) {
  return '<article class="sl" data-id="' + p.id + '">' +
    ...
    /* What this row asks of the student, rather than how it is constituted.
       ...
       have no public university at all — and it never told a student what
       applying would cost them. */
    '<span class="sl-tag' + (feeOf(p) === 'free' ? ' sl-free' : '') + '">'
      + ico(feeOf(p) === 'free' ? 'check' : 'star') + ' '
    ...
    '<div class="sl-go" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">' +
      (inList === 'matched'
        ...
        /* Only on a PRIVATE university. A public place is matched to the
           student's marks and put on the list by the office — the server
           refuses one added any other way, so offering the button here would
           be a button that fails. The card says who does add it instead of
           saying nothing. */
        : SHOW_INTEREST && !p.isPublic
        ? '<button type="button" class="btn btn-navy btn-sm" data-add="' + p.id + '">I am interested</button>'
        : SHOW_INTEREST
        ? '<a class="btn btn-ghost btn-sm" href="messages.html?about='
          + encodeURIComponent(p.university || p.program || p.id)
          + '">Ask my counsellor to add this</a>'
        : '') +
 ...
 */
/* BACK ON, and the rule underneath it changed.
   A student may add PRIVATE universities and only private ones — a public
   place is matched to their marks and put there by the office, and the
   server now refuses one added any other way. So this section can no longer
   contain a university they were not entitled to name, which is what it was
   switched off over. Leaving it off had its own cost: the entries still
   existed, so this screen said 17 while their applications screen and their
   counsellor both said 18 — one student, two of their own screens, two
   answers. */
const SHOW_INTEREST = true;
...
function paintMine() {
  ...
  if (SHOW_INTEREST) {
    html += '<div style="margin-top:30px">' + head('Universities you added',
      mine.length
        ? 'Private universities you picked yourself, not agreed yet. Your counsellor '
          + 'can see these and will tell you honestly which are realistic for your '
          + 'profile. Public universities are matched to your marks and added by them.'
        : 'Nothing here yet.');
    html += mine.length
      ? grid(mine, 'want')
      : '<div class="sl-empty"><b>Nothing marked yet</b>'
        + '<p>Browse the programmes tab and mark the private universities you like. Your '
        + 'counsellor sees them and can move any onto your real shortlist. The public '
        + 'ones are matched to your marks and added for you.</p>'
...
(function () {
  ...
  /* Start on the country the student told us they are going to.
     "Any destination" as an opening position mixes seven countries into one
     list, so a student going to Ireland scrolls past German universities to
...
/* An option with nothing behind it is a trap: "Public only" on Canada
   returned an empty screen for a year because Canada has no public row, and
   the student is left thinking the site is broken rather than that the
   question does not apply there. So the choices are rebuilt from what the
   chosen destination actually holds, and the whole control stands down when
   there is only one answer. */
function paintTypeChoices() {
  const c = $('#fCountry').value;
  const here = POOL.filter(p => !c || p.country === c);
  const kinds = new Set(here.map(feeOf));
  const box = $('#fType').closest('.field');
  const only = kinds.size < 2;
  if (box) box.hidden = only;
  if (only) { $('#fType').value = ''; return; }
  const keep = $('#fType').value;
  $('#fType').innerHTML = '<option value="">Every university</option>'
    + (kinds.has('free') ? '<option value="free">Fast-track — free</option>' : '')
    + (kinds.has('package') ? '<option value="package">Comprehensive filing</option>' : '');
  $('#fType').value = kinds.has(keep) ? keep : '';
}
...
['fCountry','fBand','fType','fReach','fSort'].forEach(id =>
  $('#' + id).addEventListener('change', () => {
    /* The destination decides which of the other questions are worth asking,
       so it repaints them before the list is redrawn. */
    if (id === 'fCountry') paintTypeChoices();
    shown = 12; paintBrowse();
  }));
