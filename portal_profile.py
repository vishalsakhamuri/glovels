...
const MUST = [
  ...
  {k:'alt_phone', l:'Alternate contact number', ifBlank:'ok',
   ok:v => /^\+?[\d\s().-]+$/.test(v) && v.replace(/\D/g, '').length >= 8 && v.replace(/\D/g, '').length <= 15,
   why:'Digits only, with the country code if it is not Indian.'},
  {k:'fam_phone', l:"Parent or guardian's mobile", ifBlank:'ok',
   ok:v => /^\+?[\d\s().-]+$/.test(v) && v.replace(/\D/g, '').length >= 8 && v.replace(/\D/g, '').length <= 15,
   why:'Digits only, with the country code if it is not Indian.'},
...
/* Checks that read two boxes at once — a year that has to follow another,
   an age. Same answers as the server, said first and beside the box. */
const CROSS = [
  { k:'dob', say:p => {
      const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(p.dob || '')); if (!m) return '';
      const born = new Date(+m[1], +m[2] - 1, +m[3]), t = new Date(); t.setHours(0,0,0,0);
      if (born > t) return 'The date of birth cannot be in the future.';
      if (born > new Date(t.getFullYear() - 15, t.getMonth(), t.getDate())) return 'You have to be at least 15 to apply — check the year of birth.';
      return ''; } },
  { k:'d_year', say:p => (+p.d_start > 1900 && +p.d_year > 1900 && +p.d_year < +p.d_start)
      ? 'The year of completion (' + p.d_year + ') cannot be before the year of joining (' + p.d_start + ').' : '' },
  { k:'m_year', say:p => (+p.m_start > 1900 && +p.m_year > 1900 && +p.m_year < +p.m_start)
      ? 'The year of completion (' + p.m_year + ') cannot be before the year of joining (' + p.m_start + ').' : '' },
  { k:'xii_year', say:p => (+p.x_year > 1900 && +p.xii_year > 1900 && +p.xii_year < +p.x_year)
      ? 'Class 12 (' + p.xii_year + ') cannot be passed before Class 10 (' + p.x_year + ').' : '' },
];
function firstProblem() {
  ...
  for (const c of CROSS) {
    if (here.indexOf(c.k) < 0) continue;
    const say = c.say(DB.profile);
    if (say) return { k:c.k, say };
  }
  return null;
}
