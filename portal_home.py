      ...
      <div class="p-card" style="margin-bottom:14px">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:0 14px">
          <div class="field" style="margin-bottom:10px">
            ...
            <input id="fWa" placeholder="917839399999">
            <span style="display:block;margin-top:4px;font-size:11.6px;color:var(--muted)">
              Country code, no plus, no spaces.</span></div>
          <div class="field" style="margin-bottom:10px">
            <label for="fPhone">Phone</label><input id="fPhone" placeholder="+91 78393 99999"></div>
          <div class="field" style="margin-bottom:10px">
            <label for="fEmail">Email</label><input id="fEmail" placeholder="info@glovels.com"></div>
        </div>

        <!-- The office's accounts. A link and a switch per network: the row
             in the footer of every page follows this, and the home page's
             structured data names the same accounts so Google ties them to
             the business. Hidden is hidden, not deleted. -->
        <h4 style="margin:16px 0 4px;font-size:13.5px">Social links</h4>
        <p style="margin:0 0 10px;font-size:12.4px;color:var(--muted);line-height:1.6">
          Paste the full address of each account (starting <code>https://</code>) and tick
          <b>Show</b> to put it in the footer of every page. Untick to hide it without losing
          the link. Only a link on that network's own site is accepted.</p>
        <div id="fSocial" style="display:grid;gap:8px">
          <!-- rows painted by paintSocial() -->
        </div>
      </div>
...
function paintPackages() {
  ...
  /* "CHEAPEST PACKAGE ₹0" was this line reading a broken price. p.sell is now
     false for one of those, so the figure is the cheapest thing anybody can
     actually buy — which is what the word cheapest means. */
  ...
  $('#pkgTabs').innerHTML = pk.tabs.map(t => {
    ...
    return '<div class="p-card" style="padding:0;margin-bottom:14px;overflow-x:auto">' +
      ...
      (rows.map(p =>
        ...
        /* priceBroken: marked "sold online" with a price that is not a number
           or is zero. The server has already taken it off sale rather than
           offer it at nothing, and this row is the only place anybody can put
           it right — so it says so here, in the Price column, rather than
           reading "On request" as though somebody had chosen that. */
        '<td>' + (p.priceBroken
            ? '<span class="st wait">Not on sale</span>' +
              '<br><span style="font-size:11.4px;color:#7a2118;font-weight:600">' +
              'The price is not a number. Edit it, or set this to "Booked on a call".</span>'
          : p.sell ? '<b>' + inr(p.priceInr) + '</b>' +
...
function finderFromScreen() {
  ...
  return {
    ...
    social: $$('#fSocial [data-social]').map(row => ({
      id: row.dataset.social,
...
/* One row per network the server knows. The list comes from the server so a
   network added there appears here without a second edit. */
function paintSocial(rows) {
  const box = $('#fSocial');
  if (!box) return;
  box.innerHTML = rows.map(r => '<div data-social="' + esc(r.id) + '" style="display:grid;'
    + 'grid-template-columns:130px 1fr auto;gap:10px;align-items:center">'
    + '<b style="font:700 12.6px/1.3 var(--sans)">' + esc(r.label || r.id) + '</b>'
    + '<input type="url" value="' + esc(r.url || '') + '" placeholder="https://www.' + esc(r.id) + '.com/glovels" '
    + 'style="padding:8px 10px;font:400 12.8px/1.4 var(--sans);border:1.5px solid #d8dde4;border-radius:8px">'
    + '<label style="display:flex;align-items:center;gap:6px;font:700 12px/1 var(--sans);white-space:nowrap">'
    + '<input type="checkbox"' + (r.show ? ' checked' : '') + '> Show</label></div>').join('');
}
...
document.addEventListener('click', async e => {
  ...
  if (e.target.closest('#smSave')) {
    ...
    /* SAID HERE, BEFORE THE ROUND TRIP. The server refuses this too — a check
       in a browser is a suggestion — but finding out after a save that the
       service you just typed already existed is a worse way to learn it. */
    const same = String(body.name || '').trim().toLowerCase().replace(/\s+/g, ' ');
    const clash = items.find(x => (!editingSvc || x.id !== editingSvc.id)
      && String(x.name || '').trim().toLowerCase().replace(/\s+/g, ' ') === same);
    if (clash) {
      $('#smErr').textContent = 'There is already a service called “' + clash.name
        + '”. Two with the same name are indistinguishable on the page — rename '
        + 'this one, or edit the one that exists.';
      $('#smErr').style.display = 'block';
      return;
    }
    if (editingSvc) {
